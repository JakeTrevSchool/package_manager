...
class PackageMethodTests(TestCase):
    ...
    def test_package_tags_can_be_empty(self):
        ...
        self.user = User.objects.create_user(
            username='testuser3', email='user4352@gmail.com', password='e92SinZod4')
        login = self.client.login(username='testuser3', password='e92SinZod4')
        profile = UserProfile(user=self.user, avatar="")
        profile.save()
        package = Package(
            author=profile, package_name='test67785', downloads=0, views=1)
        package.save()
...
class HomeTests(TestCase):
    ...
    def test_3_packages(self):
        ...
        package.save()
        package = Package(
            author=profile, package_name='test103', downloads=0, views=1)
        package.save()
        package = Package(
            author=profile, package_name='test0284', downloads=0, views=1)
        package.save()
        response = self.client.get(reverse('manager:index'))
    ...
    def test_2_developers(self):
        ...
        self.user2 = User.objects.create_user(
            username='testuser5', email='user4351@gmail.com', password='e92SinZod4')
...
class ExploreTests(TestCase):
    ...
    def test_explore_displays_my_packages(self):
        ...
        profile.save()
        package = Package(
            author=profile, package_name='MyNewPackage', downloads=0, views=1)
        package.save()
    ...
    def test_explore_displays_top_viewed_packages(self):
        ...
        profile.save()
        package = Package(
            author=profile, package_name='MyNewPackage', downloads=0, views=2, public=1)
        package.save()
        package = Package(
            author=profile, package_name='MyNewPackage2', downloads=0, views=1, public=1)
        package.save()
        package = Package(
            author=profile, package_name='MyNewPackage3', downloads=0, views=5, public=1)
        package.save()
        package = Package(
            author=profile, package_name='MyNewPackage4', downloads=0, views=6, public=1)
        package.save()
        package = Package(
            author=profile, package_name='MyNewPackage5', downloads=0, views=7, public=1)
        package.save()
        package = Package(
            author=profile, package_name='MostViewedPackage', downloads=0, views=8, public=1)
        package.save()
        package = Package(
            author=profile, package_name='LeastViewedPackage', downloads=0, views=0, public=1)
        package.save()
        response = self.client.get(reverse('manager:explore'))
    ...
    def test_profile_username_is_correct(self):
        ...
        self.user = User.objects.create_user(
            username='ThisIsTheUsername', email='user4352@gmail.com', password='e92SinZod4')
        login = self.client.login(
            username='ThisIsTheUsername', password='e92SinZod4')
        profile = UserProfile(user=self.user, avatar="")
        profile.save()
    ...
    def test_user_packages_appear_on_profile_page(self):
        ...
        self.user = User.objects.create_user(
            username='User123', email='User123@gmail.com', password='e92SinZod4')
        login = self.client.login(username='User123', password='e92SinZod4')
        profile = UserProfile(user=self.user, avatar="")
        profile.save()
        package = Package(author=profile, package_name='TheFirstPackage',
                          downloads=0, views=2, public=0)
        package.save()
        package = Package(author=profile, package_name='TheSecondPackage',
                          downloads=0, views=1, public=0)
        package.save()
    ...
    def test_profile_page_displays_downloads_of_packages(self):
        ...
        self.user = User.objects.create_user(
            username='User123', email='user4352@gmail.com', password='e92SinZod4')
        login = self.client.login(username='User123', password='e92SinZod4')
        profile = UserProfile(user=self.user, avatar="")
        profile.save()
        package = Package(author=profile, package_name='MyPackage',
                          downloads=10, views=20)
        package.save()
        response = self.client.get('/profile/User123')
